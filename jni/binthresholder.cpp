...
void NiblackSauvolaWolfJolion (InputArray _src, OutputArray _dst,int version,int winx, int winy, double k, double dR) {
    ...
    int y_firstth= wyh;
    int mx, my;
	LOGI("Using NiBlack sauvolo wolf method");
    ...
    for (int j = y_firstth ; j<=y_lastth; j++) {
        ...
        for (int i=0 ; i <= src.cols-winx; i++) {
            ...
            case WOLFJOLION:
                ...
                default:
                    LOGE("Unknown threshold type in ImageThresholder::surfaceNiblackImproved()");
                    exit (1);
            }
...
void BhThresholder::doThreshold(InputArray _src ,OutputArray _dst,int method)
{
    Mat src = _src.getMat();
...
//#define _BH_SHOW_IMAGE
    ...
    case OTSU :
		...
		LOGI("Using binthresholder otsu");
